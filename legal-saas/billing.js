// =====================================================================
// Villela Legal SaaS — cobrança recorrente (Mercado Pago) + ciclo de vida.
//
...
async function assinar(tenantId, planoSlug, quemEmail, baseUrl, ip) {
  ...
  const pre = await mp('/preapproval', {
    method: 'POST',
    body: JSON.stringify({
      reason: `Villela Legal — plano ${plano.nome}`,
      external_reference: `legalsaas:${tenantId}:${plano.slug}`,
      payer_email: quemEmail || t.email_contato,
      back_url: `${baseUrl}/juridico/app`,
...
function aplicarPreapproval(tenantId, preapprovalId, statusMP) {
  ...
  if (statusMP === 'authorized') {
    ...
    if (sub.status === 'ativa' || sub.status === 'pendente') {
      ...
      _notificar(`⚠️ Villela Legal SaaS: assinatura ${statusMP === 'paused' ? 'PAUSADA (inadimplência?)' : 'CANCELADA'} — tenant ${sub.tenant_id}.`);
    }
  }
}
...
async function processarWebhook(body, query) {
  try {
    ...
    if (tipo === 'subscription_preapproval' || tipo === 'preapproval') {
      ...
      const tenantId = ref.startsWith('legalsaas:') ? ref.split(':')[1] : (db.prepare('SELECT tenant_id FROM subscriptions WHERE mp_preapproval_id = ?').get(String(id)) || {}).tenant_id;
...
function processarCicloDeVida() {
  ...
  for (const t of db.prepare("SELECT id FROM tenants WHERE status = 'trial' AND trial_expira_em != '' AND trial_expira_em < ?").all(agora)) {
    ...
    _notificar(`⏰ Villela Legal SaaS: trial expirou — tenant ${t.id} bloqueado até assinar.`);
    trialsVencidos++;
  }
