// =====================================================================
// Villela Legal SaaS — repositório + MOTOR DE ENTITLEMENTS.
//
...
// e as FEATURE FLAGS efetivas. É o que gateia a entrega do produto jurídico.
//
// Números comerciais (preços/limites dos planos) são SEMENTES razoáveis,
...
// ---- catálogo de MÓDULOS do produto jurídico (chaves) ----
const MODULOS = [
  ['processos', 'Processos e andamentos'], ['prazos', 'Prazos e calculadora CPC'],
...
const LIMITES = [
  ['advogados', 'Advogados / usuários'], ['processos_ativos', 'Processos ativos'],
  ['ia_consultas_mes', 'Consultas de IA / mês'], ['armazenamento_mb', 'Armazenamento (MB)'],
  ['clientes_portal', 'Clientes no portal'],
];

// ---- FEATURE FLAGS globais (seed) ----
const FLAGS_SEED = [
  ['ia_direta', 'IA em modo direto', 'Respostas de IA na hora (sem fila do agente local).', 0],
  ['api_publica', 'API pública', 'Acesso à API por chave.', 0],
  ['white_label', 'Marca própria (white-label)', 'Remove a marca Villela do portal do cliente.', 0],
  ['coleta_automatica', 'Coleta automática diária', 'Rotina server-side de DataJud/DJEN.', 0],
];

// ---- PLANOS (seed; preços PROVISÓRIOS, editáveis no painel) ----
const PLANOS_SEED = [
  {
    slug: 'trial', nome: 'Trial (14 dias)', descricao: 'Avaliação completa por 14 dias.',
    preco_centavos: 0, ordem: 0,
    limites: { advogados: 2, processos_ativos: 20, ia_consultas_mes: 30, armazenamento_mb: 500, clientes_portal: 5 },
    modulos: MODULOS_KEYS, flags: { ia_direta: false, api_publica: false, white_label: false, coleta_automatica: true },
  },
  {
  ...
  {
    slug: 'escritorio', nome: 'Escritório', descricao: 'Banca estabelecida: tudo liberado, API e marca própria.',
    preco_centavos: 79900, ordem: 3,
    limites: { advogados: 15, processos_ativos: 2000, ia_consultas_mes: 2000, armazenamento_mb: 51200, clientes_portal: 1000 },
    modulos: MODULOS_KEYS, flags: { ia_direta: true, api_publica: true, white_label: true, coleta_automatica: true },
  },
  {
    slug: 'enterprise', nome: 'Enterprise', descricao: 'Grandes escritórios e departamentos jurídicos — sob consulta.',
    preco_centavos: 0, ordem: 4,
    limites: { advogados: 0, processos_ativos: 0, ia_consultas_mes: 0, armazenamento_mb: 0, clientes_portal: 0 }, // 0 = ilimitado (enterprise)
    modulos: MODULOS_KEYS, flags: { ia_direta: true, api_publica: true, white_label: true, coleta_automatica: true },
  },
];
...
const Tenants = {
  ...
  atualizar(id, d, quem) {
    const t = db.prepare('SELECT * FROM tenants WHERE id = ?').get(s(id, 40));
    if (!t) throw new Error('Escritório não encontrado.');
    for (const c of ['nome', 'cnpj', 'oab_secional', 'email_contato', 'telefone', 'obs']) if (d[c] != null) t[c] = s(d[c], c === 'obs' ? 1000 : 200);
    db.prepare('UPDATE tenants SET nome=?, cnpj=?, oab_secional=?, email_contato=?, telefone=?, obs=?, atualizado_em=? WHERE id=?')
      .run(t.nome, t.cnpj, t.oab_secional, t.email_contato, t.telefone, t.obs, nowISO(), id);
...
const Leads = {
  criar(d) {
    const id = novoId();
    db.prepare('INSERT INTO leads (id, nome, escritorio, email, telefone, plano, mensagem, status, criado_em) VALUES (?,?,?,?,?,?,?,?,?)')
      .run(id, s(d.nome, 120), s(d.escritorio, 200), s(d.email, 120), s(d.telefone, 20), s(d.plano, 60), s(d.mensagem, 2000), 'novo', nowISO());
    return id;
  },
