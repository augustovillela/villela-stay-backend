// =====================================================================
// Villela Legal SaaS — Web Push do painel do assinante (PWA).
// Assinaturas na tabela push_subs do banco do produto; envio pelo helper
// compartilhado ../push-saas.js (VAPID por env). Envio é sempre
// best-effort: nunca bloqueia a rota.
// =====================================================================
'use strict';
...
const Push = {
  ...
  salvar(tenantId, userId, sub) {
    ...
    db.prepare(`INSERT INTO push_subs (endpoint, tenant_id, user_id, dados, criado_em) VALUES (?, ?, ?, ?, ?)
      ...
      .run(sub.endpoint, tenantId, userId, j.str(sub), nowISO());
  },
  ...
  async notificarTenant(tenantId, payload) {
    return Push._enviar(db.prepare('SELECT dados FROM push_subs WHERE tenant_id = ?').all(tenantId), payload);
  },
