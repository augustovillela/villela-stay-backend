// =====================================================================
// Villela CRM — repositório do CONTROL PLANE + motor de entitlements.
//
// Entitlements: dado o plano do tenant (+ overrides em tenant_settings),
// resolve MÓDULOS, LIMITES e FLAGS efetivos — é o que gateia a entrega
// do CRM a cada empresa assinante. Padrão idêntico ao vsm/repo.js.
//
// Números comerciais (preços/limites) são SEMENTES provisórias, editáveis
// no painel da plataforma — o Augusto define os finais.
// =====================================================================
'use strict';
...
// ---- catálogo de MÓDULOS do CRM (chaves) ----
const MODULOS = [
  ['dashboard', 'Dashboard comercial'], ['contatos', 'Leads e contatos'],
  ['empresas', 'Empresas (B2B)'], ['funis', 'Funis e Kanban'],
  ['oportunidades', 'Oportunidades'], ['tarefas', 'Tarefas e follow-ups'],
  ['templates', 'Templates de mensagens'], ['propostas', 'Propostas comerciais'],
  ['campanhas', 'Campanhas e segmentação'], ['automacoes', 'Automações'],
  ['ia', 'Agentes de IA'], ['relatorios', 'Relatórios avançados'],
  ['importacao', 'Importação/Exportação'], ['api', 'API pública e webhooks'],
];
const MODULOS_KEYS = MODULOS.map(m => m[0]);

// ---- catálogo de LIMITES (métrica → rótulo) ----
const LIMITES = [
  ['contatos', 'Contatos'], ['usuarios', 'Usuários da equipe'],
  ['funis', 'Funis de venda'], ['campanhas_mes', 'Campanhas / mês'],
  ['ia_mes', 'Execuções de IA / mês'], ['templates', 'Templates'],
];

// ---- FEATURE FLAGS globais (seed) ----
const FLAGS_SEED = [
  ['automacoes', 'Automações', 'Regras automáticas de follow-up e priorização.', 0],
  ['ia', 'Agentes de IA', 'Agentes de qualificação, follow-up, reativação e análise.', 0],
  ['api_publica', 'API pública', 'Chaves de API + webhook de entrada de leads.', 0],
  ['whatsapp_api', 'WhatsApp API', 'Envio via WhatsApp Business API (quando conectado).', 0],
  ['white_label', 'Marca própria (white-label)', 'Remove a marca Villela do painel.', 0],
];
...
const PLANOS_SEED = [
  ...
  },
  {
    ...
    modulos: MODULOS_KEYS,
    flags: { automacoes: true, ia: true, api_publica: true, whatsapp_api: true, white_label: true },
  },
];

// papéis do produto e o que cada um pode (validação no backend)
const PAPEIS = ['owner', 'admin', 'gestor', 'vendedor', 'atendente', 'financeiro', 'marketing', 'leitura'];
const PODE = {
  gerir_conta: ['owner', 'admin'],                                  // plano, cobrança, usuários, config
  editar: ['owner', 'admin', 'gestor', 'vendedor', 'atendente', 'marketing'], // criar/editar contatos, oportunidades, tarefas
  campanhas: ['owner', 'admin', 'gestor', 'marketing'],
  propostas: ['owner', 'admin', 'gestor', 'vendedor'],
  financeiro: ['owner', 'admin', 'gestor', 'financeiro'],
};
const podePapel = (papel, acao) => (PODE[acao] || []).includes(papel);
...
function semear() {
  ...
  for (const p of PLANOS_SEED) {
    ...
    if (existe) db.prepare('UPDATE plans SET preco_centavos = ? WHERE slug = ?').run(preco, p.slug);
  }
...
const Planos = {
  listar({ incluirInativos = false } = {}) {
    return db.prepare(`SELECT * FROM plans ${incluirInativos ? '' : 'WHERE ativo = 1'} ORDER BY ordem`).all().map(hidratarPlano);
  },
...
// =====================================================================
// TENANTS (empresas assinantes do CRM)
// =====================================================================
const DIAS_TRIAL = parseInt(process.env.CRM_TRIAL_DIAS, 10) || 14;
const Tenants = {
  ...
  porSlug(slug) {
    const t = db.prepare('SELECT id FROM tenants WHERE slug = ?').get(s(slug, 60));
    return t ? Tenants.obter(t.id) : null;
  },
  // cria empresa + usuário owner + assinatura trial + provisiona o CRM (funis/templates seed)
  criar(d, quem) {
    ...
    const plano = Planos.porSlug(s(d.plano || 'trial', 60)) || Planos.porSlug('trial');
    return transacao(() => {
      const id = novoId(); const agora = nowISO();
      let slug = s(d.slug, 60) || slugify(nome); let n = 1;
      ...
      const statusIni = d.status_inicial === 'ativa' ? 'ativa' : (ehTrial ? 'trial' : 'ativa');
      db.prepare(`INSERT INTO tenants (id, slug, nome, cnpj, site, email_contato, telefone, status, plan_id, trial_expira_em, origem, obs, criado_em, criado_por)
        ...
        .run(id, slug, nome, s(d.cnpj, 20), s(d.site, 200), email, s(d.telefone, 20),
          statusIni, plano.id, statusIni === 'trial' ? trialAte : '', s(d.origem, 30) || 'manual', s(d.obs, 1000), agora, s(quem, 60));
      db.prepare('INSERT INTO tenant_users (id, tenant_id, nome, email, papel, ativo, criado_em) VALUES (?,?,?,?,?,1,?)')
        .run(novoId(), id, s(d.nome_responsavel || nome, 120), email, 'owner', agora);
      db.prepare('INSERT INTO subscriptions (id, tenant_id, plan_id, status, ciclo, inicio, proximo_venc, criado_em) VALUES (?,?,?,?,?,?,?,?)')
        .run(novoId(), id, plano.id, statusIni === 'trial' ? 'trial' : 'ativa', 'mensal', agora, statusIni === 'trial' ? trialAte : '', agora);
      // provisiona o CRM do tenant (config + funis padrão + templates seed)
      require('./app-repo').provisionar(id, { vertical: s(d.vertical, 30) });
      evento(id, 'tenant.criado', slug, { plano: plano.slug, trial: statusIni === 'trial' });
  ...
  // usuários da empresa (papéis do produto)
  addUsuario(tenantId, d, quem) {
    const email = s(d.email, 120).toLowerCase();
    if (!email.includes('@')) throw new Error('E-mail inválido.');
    if (db.prepare('SELECT 1 FROM tenant_users WHERE lower(email) = ?').get(email)) throw new Error('Já existe um usuário com este e-mail.');
    const lim = dentroLimite(tenantId, 'usuarios');
    if (!lim.ok) throw new Error(`Limite de usuários do plano atingido (${lim.limite}). Faça upgrade.`);
    const papel = PAPEIS.includes(d.papel) ? d.papel : 'vendedor';
    const id = novoId();
    db.prepare('INSERT INTO tenant_users (id, tenant_id, nome, email, papel, ativo, criado_em) VALUES (?,?,?,?,?,1,?)')
      .run(id, s(tenantId, 40), s(d.nome, 120), email, papel, nowISO());
    Uso.set(tenantId, 'usuarios', db.prepare('SELECT COUNT(*) n FROM tenant_users WHERE tenant_id = ? AND ativo = 1').get(tenantId).n);
    Auditoria.registrar({ tenant_id: tenantId, quem, acao: 'usuario.criar', entidade: 'tenant_users', entidade_id: id, detalhe: email + ':' + papel });
    return id;
  },
  mudarUsuario(tenantId, uid, d, quem) {
    const u = db.prepare('SELECT * FROM tenant_users WHERE id = ? AND tenant_id = ?').get(s(uid, 40), s(tenantId, 40));
    if (!u) throw new Error('Usuário não encontrado.');
    const papel = d.papel != null && PAPEIS.includes(d.papel) ? d.papel : u.papel;
    const ativo = d.ativo != null ? (d.ativo ? 1 : 0) : u.ativo;
    if (u.papel === 'owner' && (papel !== 'owner' || !ativo)) throw new Error('O owner não pode ser rebaixado/desativado por aqui.');
    db.prepare('UPDATE tenant_users SET nome = ?, papel = ?, ativo = ? WHERE id = ?').run(d.nome != null ? s(d.nome, 120) : u.nome, papel, ativo, uid);
    Uso.set(tenantId, 'usuarios', db.prepare('SELECT COUNT(*) n FROM tenant_users WHERE tenant_id = ? AND ativo = 1').get(tenantId).n);
    Auditoria.registrar({ tenant_id: tenantId, quem, acao: 'usuario.editar', entidade: 'tenant_users', entidade_id: uid });
  },
};
...
function entitlements(tenantId) {
  ...
  return {
    ...
    modulos: [...modulos], limites, flags, trial_expira_em: t.trial_expira_em || '',
  };
}
...
// 0 = ilimitado. Retorna {ok, limite, usado}
function dentroLimite(tenantId, metrica, incremento = 1) {
  ...
  if (limite === 0) return { ok: true, limite: 0, usado };
...
const Uso = {
  ...
  atual(tenantId, metrica, periodo = periodoAtual()) {
    // métricas de ESTOQUE (contatos/usuarios/funis/templates) não zeram por mês:
    // usa o valor mais recente registrado em qualquer período.
    const estoque = ['contatos', 'usuarios', 'funis', 'templates'];
    if (estoque.includes(metrica)) {
      const r = db.prepare('SELECT quantidade FROM usage_records WHERE tenant_id = ? AND metrica = ? ORDER BY periodo DESC LIMIT 1').get(s(tenantId, 40), s(metrica, 40));
...
const Tickets = {
  ...
  responder(id, { texto, lado, autor }) {
    const t = db.prepare('SELECT id FROM tickets WHERE id = ?').get(s(id, 40));
    ...
    db.prepare('UPDATE tickets SET status = ?, atualizado_em = ? WHERE id = ?').run(lado === 'plataforma' ? 'respondido' : 'em_andamento', nowISO(), id);
    return mid;
  },
...
// =====================================================================
// LEADS da landing + EVENTOS + AUDITORIA + DASHBOARD da plataforma
// =====================================================================
const Flags = { listar: () => db.prepare('SELECT * FROM feature_flags ORDER BY chave').all() };

const SaasLeads = {
  criar(d) {
    const id = novoId();
    db.prepare('INSERT INTO saas_leads (id, nome, empresa, email, telefone, plano, mensagem, status, criado_em) VALUES (?,?,?,?,?,?,?,?,?)')
      .run(id, s(d.nome, 120), s(d.empresa, 200), s(d.email, 120), s(d.telefone, 20), s(d.plano, 60), s(d.mensagem, 2000), 'novo', nowISO());
    return id;
  },
  listar: (limite = 100) => db.prepare('SELECT * FROM saas_leads ORDER BY criado_em DESC LIMIT ?').all(Math.min(Number(limite) || 100, 300)),
  status(id, st) { db.prepare('UPDATE saas_leads SET status = ? WHERE id = ?').run(s(st, 20), s(id, 40)); },
};
...
const Dashboard = {
  plataforma() {
    ...
    return {
      ...
      mrr_centavos: mrr, arr_centavos: mrr * 12,
...
module.exports = {
  semear, slugify, MODULOS, MODULOS_KEYS, LIMITES, DIAS_TRIAL, PAPEIS, podePapel,
  Planos, Tenants, salvarSettings, entitlements, podeModulo, dentroLimite, flag,
  Uso, Tickets, Flags, SaasLeads, evento, Eventos, Auditoria, Dashboard,
};
