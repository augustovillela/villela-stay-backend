...
> ⚠️ **Esta pasta é a do SITE.** O site estático não enxerga o backend: o build copia
> `site/assets/brand/` para `dist/assets/brand/`. Existe uma cópia gêmea em
> `backend/assets/brand/musique/`, servida pelo backend. **Mexeu numa, mexa na outra** — senão o
> card na home vai ao ar com a imagem quebrada, e um card sem imagem parece produto que não existe.
