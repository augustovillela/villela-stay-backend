// =====================================================================
// Musique — fila durável (ADR-0003). Espelha o growth/fila.js, que já
// provou o desenho, e a disciplina do origena/fila.js.
//
...
//
// QUEM CONSOME (ADR-0006): hoje é o próprio processo web, e SÓ a fila
// `rapida`. No Render o disco de um serviço não é acessível por outro
// serviço, e o banco da Musique é SQLite no disco — um worker separado
// subiria sem enxergar a tabela `jobs`. A fila `cara` (áudio pesado,
// GPU) está TRAVADA em `enfileirar` até existir worker de verdade:
// melhor recusar na cara do desenvolvedor do que deixar job apodrecendo
// pendente para sempre.
//
// ⚠️ Handler com nome diferente do tipo enfileirado falha em SILÊNCIO em
// muitas filas. Aqui não: vai para a DLQ na primeira tentativa e loga
// alto — repetir 5 vezes um job sem handler só atrasa o diagnóstico.
// =====================================================================
'use strict';
const { db, nowISO, novoId, j } = require('./db');

const handlers = new Map();                       // tipo → fn(payload, job)
const BASE_MS = Number(process.env.MUSIC_FILA_BACKOFF_MS || 2000);
// Liga a fila `cara` — só faz sentido quando existir um consumidor
// dedicado (worker com acesso ao banco). Ver ADR-0006.
const FILA_CARA_LIBERADA = process.env.MUSIC_FILA_CARA === 'on';
...
function enfileirar({
  ...
  if (fila === 'cara' && !FILA_CARA_LIBERADA) {
    // Trava deliberada (ADR-0006). Sem worker dedicado, um job caro
    // ficaria pendente para sempre — e pendente-para-sempre não parece
    // erro, o que é o pior jeito de falhar.
    throw new Error('fila.enfileirar: a fila "cara" exige worker dedicado, que ainda não existe '
      + '(ADR-0006). Enquanto isso, só a fila "rapida" é consumida.');
  }
  const id = novoId();
  try {
...
function falhar(job, mensagem, { semHandler = false, permanente = false } = {}) {
  ...
  // aqui faria a 1ª falha contar como 2 e mandaria para a DLQ cedo
  // demais, encurtando as tentativas pela metade em silêncio.
  ...
  if (esgotou) {
    ...
    console.error(`[music/fila] job ${job.id} (${job.tipo}) foi para a DLQ: ${erro}`);
  } else {
