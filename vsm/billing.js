// =====================================================================
// Villela Stay Manager (VSM) — cobrança recorrente (Mercado Pago) + ciclo de vida.
//
// Modelo: assinatura MENSAL via MP *preapproval* (recorrência hospedada
// no MP). mpFetch é injetado pelo server.js (mesma infra dos outros SaaS).
// Ciclo: trial → (assinar) pendente → ativa → inadimplente → suspensa /
// cancelada. Sem MP configurado, o painel da plataforma ainda gerencia
// tudo manualmente (marcar pago, suspender, reativar).
//
// Upgrade/downgrade: troca o plano do tenant; se houver assinatura MP
// ativa, exige cancelar e reassinar (limitação do preapproval do MP) —
// o painel avisa. Downgrade sem cobrança online é imediato.
// =====================================================================
'use strict';
...
// estado de cobrança para o painel do assinante
...
// inicia assinatura online (preapproval) — retorna link de checkout do MP
async function assinar(tenantId, planoSlug, quemEmail, baseUrl, ip) {
  ...
  const pre = await mp('/preapproval', {
    method: 'POST',
    body: JSON.stringify({
      reason: `Villela Stay Manager — plano ${plano.nome}`,
      external_reference: `vsm:${tenantId}:${plano.slug}`,
      payer_email: quemEmail || t.email_contato,
      back_url: `${baseUrl}/gestao/app`,
  ...
  db.prepare('UPDATE tenants SET plan_id = ? WHERE id = ?').run(plano.id, String(tenantId)); // reflete o plano escolhido
...
// aplica o resultado de um preapproval do MP (status: authorized|paused|cancelled|pending)
function aplicarPreapproval(tenantId, preapprovalId, statusMP) {
  ...
  if (statusMP === 'authorized') {
    ...
    _notificar(`💚 Villela Stay Manager: novo assinante pagante — tenant ${sub.tenant_id}.`);
    ...
    if (sub.status === 'ativa' || sub.status === 'pendente') {
      ...
      _notificar(`⚠️ Villela Stay Manager: assinatura ${statusMP === 'paused' ? 'PAUSADA (inadimplência?)' : 'CANCELADA'} — tenant ${sub.tenant_id}.`);
    }
  }
}
...
// webhook do MP (assinatura recorrente / pagamento)
async function processarWebhook(body, query) {
  try {
    ...
    if (tipo === 'subscription_preapproval' || tipo === 'preapproval') {
      ...
      const tenantId = ref.startsWith('vsm:') ? ref.split(':')[1] : (db.prepare('SELECT tenant_id FROM subscriptions WHERE mp_preapproval_id = ?').get(String(id)) || {}).tenant_id;
      ...
      const tenantId = ref.startsWith('vsm:') ? ref.split(':')[1] : '';
...
// ---- DUNNING / trial: rotina diária de ciclo de vida ----
function processarCicloDeVida() {
  const agora = nowISO();
  let trialsVencidos = 0, suspensos = 0;
  // trial vencido → inadimplente (bloqueia entrega; ainda recuperável assinando)
  for (const t of db.prepare("SELECT id FROM tenants WHERE status = 'trial' AND trial_expira_em != '' AND trial_expira_em < ?").all(agora)) {
    ...
    _notificar(`⏰ Villela Stay Manager: trial expirou — tenant ${t.id} bloqueado até assinar.`);
    trialsVencidos++;
  }
  // inadimplente há +7 dias → suspensa
  ...
  repo.evento('', 'ciclo.diario', '', { trialsVencidos, suspensos });
  return { trials_vencidos: trialsVencidos, suspensos };
}
