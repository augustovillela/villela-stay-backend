// =====================================================================
// Villela Stay Manager (VSM) — API do ASSINANTE (operação), sessão própria.
// Cookie `vsm_sess` restrito ao path /gestao. Isolado do Portal Staff e
// dos demais SaaS. Só o control plane comercial (plano, uso, suporte); o
// app de gestão real multi-tenant é o marco 2 (ponte de acesso).
// =====================================================================
'use strict';
...
function registrarRotasCliente(app, { jwtSecret, enviarEmail }) {
  ...
  app.post('/gestao/api/login', h(async (req, res) => {
    const ip = ipDe(req);
    ...
    res.cookie(COOKIE, token, { httpOnly: true, secure: seguro, sameSite: 'lax', maxAge: 30 * 24 * 3600 * 1000, path: '/gestao' });
    res.json({ ok: true });
  }));
  app.post('/gestao/api/logout', (req, res) => { res.clearCookie(COOKIE, { path: '/gestao' }); res.json({ ok: true }); });

  app.post('/gestao/api/definir-senha', h(async (req, res) => {
    const ip = ipDe(req);
    if (bloqueado(ip)) return res.status(429).json({ erro: 'Muitas tentativas. Tente em 15 minutos.' });
    ...
    if (dec.tipo !== 'vsm-setup') return res.status(400).json({ erro: 'Link inválido.' });
  ...
  app.get('/gestao/api/me', requireAssinante, h(async (req, res) => {
    ...
    res.json({
      ...
      operacao: { nome: t.nome, status: t.status, slug: t.slug },
  ...
  app.get('/gestao/api/cobranca', requireAssinante, h(async (req, res) => {
    res.json(billing.estado(req.assinante.tenant_id));
  }));
  app.post('/gestao/api/cobranca/assinar', requireAssinante, h(async (req, res) => {
    if (req.assinante.papel !== 'admin') return res.status(403).json({ erro: 'Apenas o administrador da operação assina.' });
  ...
  app.post('/gestao/api/cobranca/cancelar', requireAssinante, h(async (req, res) => {
  ...
  // suporte / tickets
  app.get('/gestao/api/tickets', requireAssinante, h(async (req, res) => {
    res.json({ tickets: repo.Tickets.listar({ tenant_id: req.assinante.tenant_id }) });
  }));
  app.get('/gestao/api/tickets/:id', requireAssinante, h(async (req, res) => {
  ...
  app.post('/gestao/api/tickets', requireAssinante, h(async (req, res) => {
    const id = repo.Tickets.abrir(req.assinante.tenant_id, req.body || {}, req.assinante.email);
    res.json({ ok: true, id });
  }));
  app.post('/gestao/api/tickets/:id/responder', requireAssinante, h(async (req, res) => {
    const t = repo.Tickets.obter(req.params.id);
    if (!t || t.tenant_id !== req.assinante.tenant_id) return res.status(404).json({ erro: 'Ticket não encontrado.' });
  ...
  app.post('/gestao/api/workspaces', requireAssinante, h(async (req, res) => {
