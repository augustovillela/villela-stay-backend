// =====================================================================
// Villela Projects & Events — Fase 8: API pública + webhooks de saída.
//
// Portado do Villela Docs. API v1 (/vpe/api/v1/*) autenticada por chave
// (Authorization: Bearer vp_... ou X-Api-Key). Regras: (1) o plano precisa
// ter api=true (Business/Enterprise); (2) rate limit por MINUTO por chave
// (VPE_API_RPM, padrão 120) + consumo em usage_records (api_chamadas);
// (3) a chave lê o portfólio/eventos e cria projetos/eventos — nunca mexe
// em usuários/billing.
//
// Webhooks de saída (integração Make/n8n/Zapier): o tenant cadastra URL +
// eventos + recebe um secret; cada evento vira uma entrega assinada
// (X-VPE-Signature = HMAC-SHA256 do corpo) com retentativa (1/5/30min).
// SSRF: só https:// (http só em dev) e nunca hosts internos em produção.
// =====================================================================
'use strict';
...
const s = repo.s;
const RPM = parseInt(process.env.VPE_API_RPM, 10) || 120;

// ------------------------------------------------------------ chaves
function criarChave(tenantId, nome, ator, ip) {
  if (!s(nome, 60)) throw new Error('Dê um nome à chave (ex.: "ERP financeiro").');
  repo.exigirRecurso(tenantId, 'api', 'API pública');
  const bruta = 'vp_' + crypto.randomBytes(24).toString('base64url');
  const id = novoId();
...
function requireApiKey(req, res, next) {
  ...
  if (!bruta.startsWith('vp_')) return res.status(401).json({ erro: 'Chave de API ausente (Authorization: Bearer vp_... ou X-Api-Key).' });
  ...
  if (!repo.recursoLiberado(k.tenant_id, 'api')) return res.status(403).json({ erro: `O plano ${plano ? plano.nome : 'atual'} não inclui API — faça upgrade para Business ou Enterprise.` });
...
function registrarApiPublica(app, { express }) {
  ...
  v1.get('/ping', (req, res) => res.json({ ok: true, produto: 'villela-projects', tenant: t(req), chave: req.apiKey.prefixo }));
  v1.get('/projetos', h(async (req, res) => res.json({ projetos: repo.listarProjetos(t(req), { status: req.query.status, estagio: req.query.estagio, categoria: req.query.categoria, busca: req.query.busca }) })));
  v1.post('/projetos', h(async (req, res) => res.json({ ok: true, projeto: repo.criarProjeto(t(req), req.body || {}, req.apiAtor, 'api') })));
  v1.get('/projetos/:id', h(async (req, res) => res.json({ projeto: repo.obterProjeto(t(req), req.params.id) })));
  v1.get('/eventos', h(async (req, res) => res.json({ eventos: eventos.listarEventos(t(req), { status: req.query.status, tipo: req.query.tipo }) })));
  v1.post('/eventos', h(async (req, res) => res.json({ ok: true, evento: eventos.criarEvento(t(req), req.body || {}, req.apiAtor, 'api') })));
  v1.get('/eventos/:id', h(async (req, res) => res.json({ evento: eventos.obterEvento(t(req), req.params.id) })));
  v1.get('/crm/deals', h(async (req, res) => res.json({ deals: comercial.listarDeals(t(req), { status: req.query.status }) })));
  v1.get('/uso', h(async (req, res) => res.json({ uso: repo.usoDoMes(t(req)), plano: (repo.planoDoTenant(t(req)) || {}).slug })));
  app.use('/vpe/api/v1', v1);
}
...
const EVENTOS = ['projeto.criado', 'evento.confirmado', 'deal.ganho', 'proposta.aprovada', 'contrato.aceito'];
function criarWebhook(tenantId, { url, eventos }, ator, ip) {
  repo.exigirRecurso(tenantId, 'api', 'Integrações por webhook');
...
const listarWebhooks = (tenantId) =>
  db.prepare('SELECT id, url, eventos, ativo, criado_em FROM webhook_subscriptions WHERE tenant_id = ? ORDER BY criado_em DESC').all(String(tenantId))
    .map(w => ({ ...w, eventos: j.parse(w.eventos, []), ativo: !!w.ativo }));
...
// Emite um evento: 1 entrega por assinatura interessada (não bloqueia quem chamou).
function emitir(tenantId, evento, dados) {
  ...
  } catch (e) { console.error('[vpe webhooks] emitir:', e.message); }
}
  ...
  for (const d of pendentes) {
    ...
    try {
      const r = await fetch(sub.url, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-VPE-Event': d.evento, 'X-VPE-Signature': 'sha256=' + assinatura },
