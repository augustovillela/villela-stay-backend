...
function registrarRotasApi(app, { express, auth, notificar }) {
  ...
  r.post('/convites', requireTenant, requirePerm('gerir_usuarios'), h(async (req, res) => {
    ...
    res.json({ ok: true, convite: { id: conv.id, email: conv.email }, link }); // quem convida entrega o link (e-mail automático: Fase 2)
  }));
