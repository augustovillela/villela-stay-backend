...
function construirItensMenu() {
  ...
  if (tem('financeiro') || tem('ceo') || tem('ti')) prod.push({ id: 'finance', rot: '💰 Villela Finance', sub: 'ERP financeiro (B2B)' });
  ...
  // O Cozinhe roda em OUTRO serviço no Render e ainda não tem API de
  // administração — o painel dele é aplicação web com login próprio, em /admin.
  // A aba existe assim mesmo (app-cozinhe.js): embute esse painel e deixa o
  // "abrir em nova aba" à vista, porque a sessão é do Cozinhe e moldura de
  // outro site pode barrar o cookie. Quando a API existir
  // (docs\PROMPT_MASTER_COZINHE_ADMIN.md), troca-se só a tela.
  if (tem('ti') || tem('ceo')) prod.push({ id: 'cozinhe', rot: '🍲 Cozinhe', sub: 'Receitas — Villela Table' });
  // Musique (15o produto). Fase 0 no ar: fundacoes, sem as telas de estudo.
  // A aba existe desde ja porque fila e IA precisam ser OBSERVAVEIS desde o
  // primeiro job — DLQ que ninguem ve e falha silenciosa com outro nome.
  if (tem('ti') || tem('ceo')) prod.push({ id: 'music', rot: '🎵 Musique', sub: 'Musica — Villela Music' });
  for (const x of prod) itens.push({ ...x, produto: true });
  // ---- PROJETOS (em andamento, ainda não são produto) ---------------------
  // Ficam no MENU DA ESQUERDA de propósito, e não no ⚡ Sistemas: aquele balde
  // é dos SaaS que já se vendem. Projeto entra aqui enquanto falta coisa para
  // terminar, e sai daqui no dia em que virar produto.
  const projetos = [];
  if (ehAdmin || tem('ti') || tem('ceo')) projetos.push({ id: 'drone', rot: '🚁 Simulador de Drone' });
  if (projetos.length) itens.push({ grupo: 'Projetos' }, ...projetos);
  // Voz fica em Operacao, e nao em Produtos: nao e um SaaS que se vende,
  // e uma forma de OPERAR a casa. E a tela existe tambem porque a pagina
  ...
  // Avisos aos usuários de TODOS os sistemas (app, e-mail, WhatsApp). Só admin:
  // é mensagem a cliente real.
  if (ehAdmin) itens.push({ id: 'comunicados', rot: '📣 Comunicados' });
  // Conversas que os clientes abrem pelo botão de suporte dentro de cada sistema.
  if (ehAdmin) itens.push({ id: 'suporte-sistemas', rot: '💬 Suporte dos sistemas' });
...
const VERTICAL_DA_SECAO = {
  ...
  music: 'grupo',    // marca Musique e PROVISORIA (ADR-0005): sem paleta propria ate haver book
  drone: 'grupo',    // projeto, ainda sem marca propria — navy do grupo
  voz: 'grupo',      // forma de operar a casa, nao produto — navy do grupo
