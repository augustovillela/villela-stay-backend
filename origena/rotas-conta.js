...
function registrarRotasConta(app, { jwtSecret }) {
  ...
  app.post(`${R}/cadastrar`, h(async (req, res) => {
    ...
    if (nome.length < 2) return res.status(400).json({ erro: 'Diga seu nome.' });
    ...
    if (!d.aceito_termos) return res.status(400).json({ erro: 'É preciso aceitar os termos de uso.' });

    if (await Users.porEmail(mail)) {
      // Não confirmamos que o e-mail existe (evita enumerar contas).
      return res.status(202).json({ ok: true, mensagem: 'Se este e-mail estiver livre, enviamos um link de confirmação.' });
    }
    ...
    await emails.verificacao(mail, nome, tokenVerificacao);
    res.status(201).json({ ok: true, mensagem: 'Enviamos um link de confirmação para o seu e-mail.' });
  }));

  app.get('/origena/api/v1/conta/verificar', h(async (req, res) => {
    const u = await Users.verificarEmail(s(req.query.token, 200));
    if (!u) return res.status(400).json({ erro: 'Link inválido ou vencido. Peça outro.' });
  ...
  app.post(`${R}/entrar`, h(async (req, res) => {
    ...
    if (await sessao.bloqueado(chaveIp) || await sessao.bloqueado(chaveEmail)) {
      return res.status(429).json({ erro: 'Muitas tentativas. Espere alguns minutos e tente de novo.' });
    }
    ...
    if (u.mfa_ativo) {
      ...
      if (!porTotp && !porBackup) {
        ...
        return res.status(401).json({ erro: 'Código de verificação incorreto.', mfa_necessario: true });
      }
  ...
  app.post(`${R}/esqueci`, h(async (req, res) => {
    ...
    if (u) {
      ...
      await emails.recuperacao(u.email, u.nome, token);
    }
    // Resposta idêntica exista ou não a conta.
    res.json({ ok: true, mensagem: 'Se houver uma conta com este e-mail, enviamos as instruções.' });
  }));
  ...
  app.post(`${R}/nova-senha`, h(async (req, res) => {
    ...
    await emails.avisoSeguranca(u.email, u.nome, 'A senha da sua conta foi alterada e todas as sessões foram encerradas.');
    sessao.encerrar(res);
    res.json({ ok: true });
  }));
  ...
  app.post(`${R}/mfa/iniciar`, sessao.requireUsuario, h(async (req, res) => {
    if (!sessao.mfaDisponivel()) {
      return res.status(503).json({ erro: 'A verificação em duas etapas não está disponível nesta instalação.' });
    }
    if (req.usuario.mfa_ativo) return res.status(409).json({ erro: 'A verificação em duas etapas já está ativa.' });
  ...
  app.post(`${R}/mfa/confirmar`, sessao.requireUsuario, h(async (req, res) => {
    ...
    if (!u.mfa_segredo_cif) return res.status(400).json({ erro: 'Comece pela geração do código.' });
    if (!sessao.conferirTOTP(sessao.decifrar(u.mfa_segredo_cif), (req.body || {}).codigo)) {
      return res.status(400).json({ erro: 'Código incorreto. Confira o horário do celular e tente de novo.' });
    }
    ...
    await emails.avisoSeguranca(u.email, u.nome, 'A verificação em duas etapas foi ativada na sua conta.');
  ...
  app.post(`${R}/mfa/desativar`, sessao.requireUsuario, h(async (req, res) => {
    ...
    if (!await sessao.conferirSenha((req.body || {}).senha, u.senha_hash)) {
      return res.status(401).json({ erro: 'Confirme sua senha para desativar.' });
    }
    await Users.desativarMFA(u.id);
    await auditar({ atorUserId: u.id, acao: 'conta.mfa_desativado', alvoTipo: 'user', alvoId: u.id, req });
    await emails.avisoSeguranca(u.email, u.nome, 'A verificação em duas etapas foi DESATIVADA na sua conta.');
    res.json({ ok: true });
  }));
