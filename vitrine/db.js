// =====================================================================
// Vitrine — camada de banco (SQLite via node:sqlite).
// Banco próprio em DATA_DIR/vitrine/ (isolado dos outros produtos).
// Sem dependência nativa (node:sqlite, Node 22+). Padrão do closet/db.js.
// =====================================================================
'use strict';
...
const MOD_DIR = path.join(DATA_DIR, 'vitrine');
fs.mkdirSync(MOD_DIR, { recursive: true });

const DB_PATH = path.join(MOD_DIR, 'vitrine.db');
...
// fase 6 (06/08/2026): Checkout Pro do MP Split
garantirColuna('payments', 'checkout_url', "TEXT NOT NULL DEFAULT ''");
garantirColuna('payments', 'dados', "TEXT NOT NULL DEFAULT ''");
...
// Código curto e legível do pedido (sem 0/O/1/I para não confundir por telefone).
const ALFA = '23456789ABCDEFGHJKLMNPQRSTUVWXYZ';
function novoCodigo(prefixo = 'VT') {
  let s = '';
...
module.exports = { db, transacao, nowISO, hojeISO, novoId, novoToken, novoCodigo, j, DATA_DIR, MOD_DIR, DB_PATH };
